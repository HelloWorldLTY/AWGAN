...
def WGAN_train(train_label,train_data,epoch,batch,lambda_1):
  ...
  # scheduler_D =torch.optim.lr_scheduler.OneCycleLR(d_optimizer,max_lr=lr,total_steps=(epoch)*(MAX_ITER//batch+1), verbose=False)
  # scheduler_G =torch.optim.lr_scheduler.OneCycleLR(g_optimizer,max_lr=lr,total_steps=(epoch)*(MAX_ITER//batch+1), verbose=False) 
...
def sequencing_train(ref_adata, batch_adata, batch_inf, epoch=100, batch=32, lambda_1=1/10):
